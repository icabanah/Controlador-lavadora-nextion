...
- Válvula Drenaje: GPIO 13 (⚠️ **Lógica inversa**: LOW=abierto/drena, HIGH=cerrado/retiene)
...
### Válvula de Drenaje - Lógica Inversa

**IMPORTANTE**: La electroválvula de drenaje tiene lógica inversa a las demás válvulas:

```cpp
// Válvula de DRENAJE (lógica inversa)
digitalWrite(DRAIN_VALVE, LOW);   // ✅ Válvula desconectada = ABIERTO (drena)
digitalWrite(DRAIN_VALVE, HIGH);  // ⛔ Válvula energizada = CERRADO (retiene agua)

// Válvulas de AGUA (lógica normal)
digitalWrite(COLD_WATER_VALVE, HIGH);  // ✅ Válvula abierta (fluye agua)
digitalWrite(COLD_WATER_VALVE, LOW);   // ⛔ Válvula cerrada
```

**Razón técnica:**
- La electroválvula de drenaje es normalmente abierta (fail-safe)
- Si pierde energía, el agua drena automáticamente (seguridad contra rebalses)
- Necesita estar energizada (HIGH) para retener el agua durante el lavado

**Implementación:**
- `openDrain()` → `digitalWrite(DRAIN_VALVE, LOW)`
- `closeDrain()` → `digitalWrite(DRAIN_VALVE, HIGH)`

**Comportamiento al inicio:**
- `resetAll()` → `emergencyShutdown()` → `openDrain()` = LOW
- Esto permite vaciar agua residual al encender el equipo
